...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const appModule_id = params.get("appModule_id");

    if (!token || !role_id || !appModule_id) return;

    setIsSsoLoading(true);
    window.history.replaceState({}, "", window.location.pathname);

    const doSso = async () => {
      const url = `${import.meta.env.VITE_API_BASE_URL}/api/sso/callback?token=${token}&role_id=${role_id}&appModule_id=${appModule_id}`;
      ...
      try {
        ...
        const { token: cbtToken, user } = data.data;
        localStorage.setItem("token", cbtToken);
        localStorage.setItem("role", user.role);
        localStorage.setItem("email", user.email);
        localStorage.setItem("nama", user.nama || user.email);

        const role = user.role?.toLowerCase();
        if (role?.includes("admin")) window.location.replace("/cbt/admin");
        else if (role?.includes("mahasiswa") || role?.includes("student"))
          window.location.replace("/cbt/student-dashboard");
        else window.location.replace("/cbt/dashboard");
      } catch (err) {
        setError("SSO gagal: " + err.message);
        setIsSsoLoading(false);
      }
    };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || err.message || "Kredensial tidak valid.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (isSsoLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
        <div className="w-10 h-10 border-4 border-[#0f4c3a] border-t-transparent rounded-full animate-spin mb-4" />
        <p className="text-sm font-bold text-slate-600">Memproses SSO...</p>
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-[#0f4c3a] relative items-center justify-center">
        ...
        <div className="relative z-10 text-center px-16">
          <motion.div
            ...
            className="mx-auto h-24 w-24 bg-white rounded-xl shadow-2xl flex items-center justify-center mb-10 border-b-4 border-[#d4af37]"
          >
            <svg
              className="w-12 h-12 text-[#0f4c3a]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <motion.h1
            ...
            className="text-4xl font-black text-white tracking-tight mb-4 leading-tight"
          >
            Sistem Ujian <br />
            Berbasis Komputer
          </motion.h1>
          <motion.div
            ...
            className="w-16 h-1 bg-[#d4af37] mx-auto mb-6"
          ></motion.div>
          ...
          <motion.p
            ...
            className="text-[#a7f3d0] text-[15px] font-medium max-w-sm mx-auto leading-relaxed"
          >
            Portal evaluasi akademik terintegrasi untuk Dosen dan Mahasiswa
        ...
        <div className="w-full max-w-[420px]">
          ...
          {error && (
            ...
            >
              ...
              <p className="text-[13px] text-red-700 font-bold leading-relaxed">
                {error}
              </p>
            </motion.div>
          )}
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <a
                  href="#"
                  className="text-[11px] font-bold text-[#0f4c3a] hover:text-[#092e23] transition-colors"
                >
                  Lupa sandi?
                </a>
              </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-[13px] font-semibold text-slate-500">
              ...
              <Link
                to="/register"
                className="text-[#0f4c3a] font-black hover:text-[#092e23] hover:underline transition-colors"
              >
                Daftar Sekarang
              </Link>
            </p>
          ...
          <div className="mt-12 text-center">
            <p className="text-[11px] font-bold text-slate-400">
              &copy; 2026 Universitas Ibn Khaldun Bogor.
              <br />
