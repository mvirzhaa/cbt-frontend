...
export default function TakeExam() {
    ...
    // 🤖 1. PERSIAPAN OTAK AI SAAT HALAMAN DIBUKA (LAZY LOAD)
    useEffect(() => {
        const loadAiModels = async () => {
            try {
                // Lazy import face-api.js agar tidak masuk main bundle
                        ...
                        <motion.div key={currentIndex} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="bg-white rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-200 min-h-full flex flex-col overflow-hidden">
                            ...
                            <div className="px-10 py-8">
                                ...
                                <div className="bg-white">
                                    ...
                                    {/* ☑️ PILIHAN GANDA MULTIPLE CHOICE (bisa pilih > 1) */}
                                    {currentQuestion.tipe_soal === 'TIPE_2' && (
                                        <div>
                                            <p className="text-[11px] text-purple-600 font-bold mb-4 bg-purple-50 px-4 py-2.5 rounded-xl border border-purple-100 flex items-center gap-2">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                Anda dapat memilih <strong>lebih dari satu</strong> jawaban yang benar.
                                            </p>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {currentQuestion.question_options?.map((opt) => {
                                                    // Parse current selections from comma-separated string
                                                    const currentSelections = (answers[currentQuestion.id] || '').split(',').filter(s => s.trim());
                                                    const isSelected = currentSelections.includes(opt.label_pilihan);
                                                    return (
                                                        <label key={opt.id} className={`flex items-start gap-5 p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 group ${isSelected ? 'bg-purple-50/50 border-purple-500 shadow-[0_0_20px_rgba(147,51,234,0.1)] ring-1 ring-purple-500' : 'bg-white border-slate-200 hover:border-purple-300 hover:bg-slate-50'}`}>
                                                            <div className={`mt-0.5 w-6 h-6 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? 'border-purple-600 bg-purple-600' : 'border-slate-300 group-hover:border-purple-400 bg-white'}`}>
                                                            ...
                                                            <input type="checkbox" checked={isSelected} onChange={() => {
                                                                const selections = (answers[currentQuestion.id] || '').split(',').filter(s => s.trim());
                                                                let newSelections;
                                                                if (selections.includes(opt.label_pilihan)) {
                                                                    newSelections = selections.filter(s => s !== opt.label_pilihan);
                                                                } else {
                                                                    newSelections = [...selections, opt.label_pilihan].sort();
                                                                }
                                                                handlePilihJawaban(currentQuestion.id, newSelections.join(','));
                                                            }} className="hidden" />
                                                            <div className="flex gap-2">
                                            ...
                                            {(answers[currentQuestion.id] || '').split(',').filter(s => s.trim()).length > 0 && (
                                                <div className="mt-4 p-3 bg-purple-50 border border-purple-200 rounded-xl">
                                                    <p className="text-[11px] font-black text-purple-700 uppercase tracking-widest">
                                                        Jawaban terpilih: {(answers[currentQuestion.id] || '').split(',').filter(s => s.trim()).join(', ')}
                                                    </p>
                                                </div>
                                            )}
                                        </div>
