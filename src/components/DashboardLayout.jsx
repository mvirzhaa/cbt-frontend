...
export default function DashboardLayout() {
  ...
  const handleLogout = async () => {
    ...
    const result = await Swal.fire({
      ...
      customClass: {
        popup:
          "rounded-3xl shadow-[0_20px_50px_rgba(8,_112,_184,_0.7)] border-0 p-0 overflow-hidden", // Bentuk popup lebih modern & shadow besar
        title: "text-2xl font-black text-slate-800 tracking-tight mt-8", // Tipografi judul yang kuat
        htmlContainer: "mb-8", // Jarak teks isi
        actions:
          "bg-slate-50 w-full py-6 px-8 flex gap-4 justify-center border-t border-slate-100 mb-0", // Container tombol di bawah
        confirmButton:
          "px-6 py-3 rounded-xl text-[13px] font-black uppercase tracking-wider shadow-lg shadow-red-500/30 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white transition-all active:scale-95 flex-1 max-w-[200px]", // Tombol konfirmasi merah gradien
        cancelButton:
          "px-6 py-3 rounded-xl text-[13px] font-black uppercase tracking-wider bg-white text-slate-600 border-2 border-slate-200 hover:bg-slate-50 transition-all active:scale-95 flex-1 max-w-[200px]", // Tombol batal minimalis
      },
      // Menambahkan sedikit tekstur halus pada background popup
      background:
        '#ffffff url("https://www.transparenttextures.com/patterns/cubes.png")',
  ...
  if (isAdmin) {
    ...
    menuItems = [
      ...
      {
        path: "/take-exam",
        label: "Ruang Ujian Terpadu",
        icon: "M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122",
      },
      {
        path: "/panduan",
        label: "Panduan & Tata Tertib",
    ...
    menuItems = [
      ...
      {
        path: "/manage-materi",
        label: "Materi Kuliah (LMS)",
        icon: "M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12",
      },
      {
        path: "/create-exam",
        label: "Penerbitan Ujian",
      ...
      {
        path: "/ai-proctoring",
        label: "Monitor AI (CCTV)",
        icon: "M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z",
      },
      {
        path: "/rekap-nilai",
        label: "Rekap Nilai Akhir",
        icon: "M9 17v-2m-2-4v-6m6 6v2m2 4v6m2-6v-6m-2 0H9m0 0H7",
      },
    ];
  }
  ...
  const lmsReturnUrl =
    (typeof window !== "undefined" &&
      sessionStorage.getItem("lms_return_url")) ||
  ...
  return (
    <div className="flex h-screen bg-[#f0f4f8] font-sans overflow-hidden text-slate-800">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-64 bg-gradient-to-b from-[#0f4c3a] to-[#092e23] text-white flex flex-col shadow-xl font-medium transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:relative lg:translate-x-0 lg:z-20`}
      >
        ...
        <Link
          to="/profile"
          className="block pt-8 pb-6 px-5 border-b border-[#16654e]/50 text-center relative z-10 group hover:bg-white/5 transition-all duration-300 cursor-pointer"
        >
          ...
          <h1 className="font-bold text-[13px] tracking-wide mb-1 leading-snug text-white group-hover:text-[#d4af37] transition-colors">
            {userName}
          </h1>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-3 relative z-10 space-y-1">
          <p className="px-3 text-[9px] font-bold text-[#6ee7b7]/70 uppercase tracking-[0.15em] mb-3">
            Navigasi Utama
          </p>
          {menuItems?.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-[12px] font-bold rounded-lg transition-all duration-300 group relative overflow-hidden ${isActive ? "bg-white/10 text-white shadow-[inset_0_0_15px_rgba(255,255,255,0.05)] border-l-[3px] border-[#d4af37]" : "text-[#a7f3d0] hover:bg-white/5 hover:text-white border-l-[3px] border-transparent"}`}
              >
                <svg
                  className={`w-4 h-4 transition-all duration-300 ${isActive ? "text-[#d4af37] drop-shadow-[0_0_5px_rgba(212,175,55,0.6)]" : "text-[#6ee7b7]/50 group-hover:text-[#d4af37]"}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
        ...
        <div className="p-4 border-t border-[#16654e]/30 relative z-10 space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 text-[12px] font-bold text-red-200 bg-red-900/20 hover:bg-red-900/40 hover:text-white rounded-lg transition-all border border-red-900/30"
          >
            <svg
              className="w-4 h-4"
              fill="none"
        ...
        <header className="h-14 bg-white/90 backdrop-blur-md border-b border-slate-200/80 flex justify-between items-center px-4 sm:px-6 z-10 shadow-sm gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-black text-slate-800 tracking-tight truncate">
                Portal Akademik CBT
              </h2>
              <p className="text-[10px] font-bold text-[#0f4c3a] uppercase tracking-widest mt-0.5 truncate">
          ...
          <div className="hidden sm:flex text-right flex-col justify-center bg-slate-100/50 px-4 py-1.5 rounded border border-slate-200/50 shrink-0">
            <span className="text-xs font-black text-slate-700 tracking-wide">
              ...
              <span className="text-[9px] text-slate-500 font-bold">WIB</span>
            </span>
